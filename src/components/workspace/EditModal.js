...
const EditModal = (props) => {
  ...
  const saveFolderName = () => {
    ...
    axios
      .post(
        url,
        { [props.type === "제작함" ? "cboxName" : "aboxName"]: folderName },
        {
  ...
  const saveMoveFolder_cbox = () => {
    ...
    axios
      .put(
        `/api/enq/move/${props.enqId}`,
        { cboxId: selectedFolder.cboxId },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  const saveMoveFolder_abox = () => {
    ...
    axios
      .put(
        `/api/wspace/ans/move/${props.ansId}`,
        { aboxId: selectedFolder.aboxId },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  const ansDelete = (confDel) => {
    ...
    axios
      .put(`/api/wspace/ans/delete/${props.ansId}`, {
        headers: { Authorization: "Bearer " + String(tokenValue) },
      })
      .then((response) => {
  ...
  const enqShare = (confShare) => {
    ...
    //e8. 설문지 공유상태 변경(샌드박스에 나타내기 위한)
    axios
      .put(`/api/enq/share/${props.enqId}`, null, {
        headers: { Authorization: "Bearer " + String(tokenValue) },
      })
      .then((response) => {
  ...
  const bring_to_cbox = () => {
    ...
    axios
      .post(
        "/api/sbox/bring",
        { cboxId: selectedFolder.cboxId, enqId: props.enqId },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  const saveModifyFName = () => {
    ...
    axios
      .put(
        url,
        { [props.type === "cbox" ? "cboxName" : "aboxName"]: modifyFName },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  const startDist = (confDist) => {
    ...
    axios
      .put(
        "/api/enq/dist/status",
        { enqId: props.enqId, enqStatus: "DIST_DONE" },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  const endDist = (confDist) => {
    ...
    axios
      .put(
        "/api/enq/dist/status",
        { enqId: props.enqId, enqStatus: "ENQ_DONE" },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  const respGPS_abox = () => {
    ...
    axios
      .post(
        "/api/wspace/gps/resp",
        { enqId: props.enqId, aboxId: selectedFolder.aboxId },
        {
          headers: { Authorization: "Bearer " + String(tokenValue) },
        }
      )
  ...
  return (
    <Wrapper>
      ...
      {props.what === "newFolder" && (
        <div>
          <h2>Create Folder</h2>
          <InputWrapper type="text" placeholder="Folder Name" value={folderName} onChange={handleFolderNameChange} />
          {/* <SaveBtn style={{marginLeft: "230px", marginTop: "5px"}} onClick={saveFolderName}>확인</SaveBtn> */}
          <BtnWrapper>
            <SaveBtn style={{ marginLeft: "160px", border: 0 }} onClick={closeModal}>
              취소
            </SaveBtn>
            <SaveBtn style={{ marginLeft: "10px" }} onClick={saveFolderName}>
              확인
            </SaveBtn>
          </BtnWrapper>
        </div>
      ...
      {props.what === "modifyEnqName" && (
        <div>
          <h2>설문지 이름 변경</h2>
          <InputWrapper type="text" placeholder={props.enqName} value={enqName} onChange={handleEnqNameChange} />
          {/* <SaveBtn style={{marginLeft: "230px", marginTop: "5px"}} onClick={modifyEnqName}>확인</SaveBtn> */}
          <BtnWrapper>
            <SaveBtn style={{ marginLeft: "160px", border: 0 }} onClick={closeModal}>
              취소
            </SaveBtn>
            <SaveBtn style={{ marginLeft: "10px" }} onClick={modifyEnqName}>
              확인
            </SaveBtn>
          </BtnWrapper>
        </div>
      ...
      {props.what === "moveFolder_cbox" && (
        <div>
          <h2>폴더 이동</h2>
          {[props.folderList[0], ...props.folderList.slice(1).sort((a, b) => a.cboxName.localeCompare(b.cboxName))].map((item) => (
            <div
              key={item.cboxId}
          ...
          <BtnWrapper>
            <SaveBtn style={{ marginLeft: "170px", border: 0 }} onClick={closeModal}>
              취소
            </SaveBtn>
      ...
      {props.what === "moveFolder_abox" && (
        <div>
          <h2>폴더 이동</h2>
          {[props.folderList[0], ...props.folderList.slice(1).sort((a, b) => a.aboxName.localeCompare(b.aboxName))].map((item) => (
            <div
              key={item.aboxId}
      ...
      {props.what === "replicateEnq" && (
        <CenteredDiv>
          ...
          <BtnWrapper>
            <SaveBtn className="cancel" style={{ marginRight: "15px", cursor: "pointer" }} onClick={closeModal}>
              취소
            </SaveBtn>
      ...
      {props.what === "bringToCbox" && (
        <div>
          <h2>
            "{props.enqName}"을 <br />내 제작함으로 가져오기
          </h2>
          {[props.folderList[0], ...props.folderList.slice(1).sort((a, b) => a.cboxName.localeCompare(b.cboxName))].map((item) => (
            <div
              key={item.cboxId}
          ...
          <BtnWrapper>
            <SaveBtn style={{ marginLeft: "8.5rem", border: 0, width: "4rem" }} onClick={closeModal}>
              취소
            </SaveBtn>
            <SaveBtn style={{ width: "5.5rem", marginRight: "0.2rem" }} onClick={bring_to_cbox}>
              가져오기
            </SaveBtn>
          </BtnWrapper>
        </div>
      ...
      {props.what === "modifyFolderName" && (
        <div>
          <h2>폴더 이름 변경</h2>
          <InputWrapper type="text" placeholder={props.boxName} value={modifyFName} onChange={handleFNameModify} />
          <BtnWrapper>
            <SaveBtn style={{ marginLeft: "160px", border: 0 }} onClick={closeModal}>
              취소
            </SaveBtn>
            <SaveBtn style={{ marginLeft: "10px" }} onClick={saveModifyFName}>
              확인
            </SaveBtn>
          </BtnWrapper>
        </div>
      ...
      {props.what === "responseGPS" && (
        <div>
          <h2>
            "{props.enqName}"을 <br />내 참여함으로 가져오기
          </h2>
          {[props.folderList[0], ...props.folderList.slice(1).sort((a, b) => a.aboxName.localeCompare(b.aboxName))].map((item) => (
            <div
              key={item.aboxId}
          ...
          <BtnWrapper>
            ...
            <SaveBtn style={{ width: "5.5rem", marginRight: "0.2rem" }} onClick={respGPS_abox}>
              가져오기
            </SaveBtn>
          </BtnWrapper>
        </div>
